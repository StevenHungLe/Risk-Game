/**
 * CSCI 2120 Fall 2014
 * Risk Game Class InitSetupView
 * Authors: Hung Le, Jeanne Vu 
 * Date: 11-25-2014
 **
 * Class:	InitSetupView - the view in which innitial set up of the game takes place
...
public class InitSetupView extends View implements Observer{
	...
	public InitSetupView(RiskGameEngine gameEngine){

		this.gameEngine = gameEngine;
		this.cbBoxesOfTerritories = new ArrayList<JComboBox>();

        // the game map
        riskBoard = new Images("src/resources/RiskBoard.jpg");
		imageComponent = new ImageComponent(riskBoard.getImage(1350,870));
			...
			if(selectedTerritory != null)
			{
				...
				if ( !(gameBoard.getUnoccupiedTerritories().isEmpty()))
				{
					...
					if( gameBoard.getUnoccupiedTerritories().size()<1)
					{
						...
						viewTitleBar.setText("Initial Set-up <Phase 2> : Choose an owned territory to add more armies!");
				...
				if ( game.getCurrentPlayer().getNumArmies() <= 0 )
				{
					...
					gameEngine.setState("playerTurnView");
				}
	...
	/**
	 * load necessary info for the view
	...
	public void loadData()
	{
		...
		this.playersList = game.getPlayersList();
          ...
          viewTitleBar = new JLabel ("Initial Set-up <Phase 1> : Choose an empty territory to claim ownership!");
          viewTitleBar.setFont( new Font ("Serif", Font.BOLD, 28) );
          titleBarJP.add(viewTitleBar);
		
          // JLabel to display player's name
		playerNameDisplay = new JLabel("Player: "+this.game.getCurrentPlayer().getName()+".");
		playerNameDisplay.setFont( new Font ("Serif", Font.BOLD, 24) );
		...
		numArmiesDisplay.setFont( new Font ("Serif", Font.BOLD, 24) );
