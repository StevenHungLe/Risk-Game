 ...
 * Date: 11-25-2014
 **
 * Class:	AttackScreen - the subScreen in which the player initiate attacks
...
public class AttackScreen extends JPanel{
	...
	public AttackScreen(RiskGameEngine gameEngine, Images riskBoard, GameGUI gameGUI)
	{
		...
		// SubScreens Panel: contains gameMap and battle result panel
		subScreensJP = new JPanel(new CardLayout());
		subScreensJPLayout= (CardLayout) subScreensJP.getLayout();

		subScreensJP.add(imageComponent,"gameMap");
		subScreensJP.add(battleResultJP,"battleResultJP");
		subScreensJPLayout.show(subScreensJP,"gameMap");


		// Add the two main panel to the attack screen
		this.add( attackSelectionsJP, BorderLayout.WEST);
		this.add( subScreensJP, BorderLayout.CENTER);
	...
	private class ButtonHandler implements ActionListener{
		...
		public void actionPerformed (ActionEvent event)
		{
			...
			{
			     // Print info to screen for monitoring purpose
				System.out.println("Before, Attacker has "+attacker.getNumArmies());
                    ...
                    // Print the roll result to screen for monitoring purpose
				...
				attackerLostLB.setText("Attacker lost "+battleResults[0]+" armies!");
	               ...
	               if ( game.getCurrentPlayer().getTerritoriesMap().containsKey(selectedDefender)) 
	               {
                    ...
                    // display the result
	               subScreensJPLayout.show(subScreensJP,"battleResultJP");
			...
			else if( event.getSource() == skipButton) // skip button
			{
			     ...
			     // switch to fortifyScreen after readjust frame size
			     gameGUI.setExtendedState(GameGUI.MAXIMIZED_BOTH); 
			     game.setState("fortifyScreen");
			}
	...
	private class ComboBoxHandler implements ActionListener{ 
     ...
     /**
	 * loads the necessary data for class attack screen
	 **/
     ...
     public void initialSetup()
	{
		...
		System.out.println("innitial setup for attack screen");
	}

     /**
	 * update every time the screen is switched to
	 **/
	public void update()
	{
	     System.out.println("updating the attack screen for the current player");
		updateAttackerCB(game.getCurrentPlayer().getTerritoriesMap().keySet().toArray());
		conqueredATerr = false;
	}
